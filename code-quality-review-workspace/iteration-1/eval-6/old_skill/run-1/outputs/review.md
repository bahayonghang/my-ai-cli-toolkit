...
Target: `skills/development-workflows/code-quality-review/evals/files/order_service.ts`
Mode: read-only code quality / maintainability review (no files modified)
...
- Recommended remediation: Import and reuse `formatMoney` from `shared/money.ts`; delete the local copy. (I could not open `shared/money.ts` in this scope — see Scope limitations — so confirm the import path and that the signature matches before removing.)
- Confidence: Medium
