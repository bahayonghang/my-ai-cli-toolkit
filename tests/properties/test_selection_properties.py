...
from tui.core.models import InstallStatus, ItemInfo, ItemType
...
def test_property_5_filter_is_case_insensitive(item_infos: list[ItemInfo], filter_text: str):
    ...
    **Validates: Requirements 11.4**
    ...
    list_view.filter_items(filter_text)
    visible_original = {item.item_name for item in list_view._get_visible_items()}

    # Apply filter with uppercase
    list_view.filter_items(filter_text.upper())
    visible_upper = {item.item_name for item in list_view._get_visible_items()}

    # Apply filter with lowercase
    list_view.filter_items(filter_text.lower())
    visible_lower = {item.item_name for item in list_view._get_visible_items()}
