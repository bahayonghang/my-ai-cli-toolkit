...
def check_command_available(command):
    ...
    try:
        subprocess.run([command, "--version"], capture_output=True, check=False, timeout=5, encoding="utf-8", errors="replace")
        return True
...
def compile_latex(source_path, output_dir, has_chinese):
    ...
    if has_chinese:
        cmd = ["latexmk", "-xelatex", f"-output-directory={output_dir}", str(source_path)]
    else:
        cmd = ["latexmk", "-pdf", f"-output-directory={output_dir}", str(source_path)]
...
def main():
    ...
    parser.add_argument(
        "--output-dir", type=str, default=None, help="Output directory (default: output/ relative to project root)"
    )
