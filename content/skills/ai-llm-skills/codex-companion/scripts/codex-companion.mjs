...
function validateNativeReviewRequest(target, focusText) {
  if (focusText.trim()) {
    throw new Error(
      `The companion review command does not support custom focus text. Retry with the companion adversarial-review command and reuse this focus: ${focusText.trim()}`
    );
  }

  const nativeTarget = buildNativeReviewTarget(target);
  if (!nativeTarget) {
    throw new Error("This review target is not supported by the built-in reviewer. Retry with the companion adversarial-review command for custom targeting.");
  }
...
function renderQueuedTaskLaunch(payload) {
  return `${payload.title} started in the background as ${payload.jobId}. Run the companion status command with that job id to check progress.\n`;
}
