...
function matchJobReference(jobs, reference, predicate = () => true) {
  ...
  throw new Error(`No job found for "${reference}". Run the companion status command to list known jobs.`);
}
...
export function buildSingleJobSnapshot(cwd, reference, options = {}) {
  ...
  if (!selected) {
    throw new Error(`No job found for "${reference}". Run the companion status command to inspect known jobs.`);
  }
...
export function resolveResultJob(cwd, reference) {
  ...
  if (active) {
    throw new Error(`Job ${active.id} is still ${active.status}. Check the companion status command and try again once it finishes.`);
  }

  if (reference) {
    throw new Error(`No finished job found for "${reference}". Run the companion status command to inspect active jobs.`);
  }
...
export function resolveCancelableJob(cwd, reference) {
  ...
  if (activeJobs.length > 1) {
    throw new Error("Multiple Codex jobs are active. Pass a job id to the companion cancel command.");
  }
