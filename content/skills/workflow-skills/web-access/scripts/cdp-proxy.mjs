...
// 要求：Chrome / Chromium 已启用 remote debugging
// Node.js 22+ 优先；低版本需要可解析的 ws 模块
...
const ENABLE_FILE_UPLOAD = process.env.CDP_ENABLE_FILE_UPLOAD === '1';
const UPLOAD_STAGE_DIR = path.resolve(
...
function ensureUploadStageDir() {
  fs.mkdirSync(UPLOAD_STAGE_DIR, { recursive: true });
  return fs.realpathSync(UPLOAD_STAGE_DIR);
}
...
function validateUploadFiles(files) {
  if (!ENABLE_FILE_UPLOAD) {
    const err = new Error('file upload disabled');
    err.statusCode = 403;
    throw err;
  }
  if (!Array.isArray(files) || files.length === 0) {
    const err = new Error('files 必须是非空数组');
    err.statusCode = 400;
    throw err;
  }

  const stageDir = ensureUploadStageDir();

  return files.map((entry) => {
    if (typeof entry !== 'string' || !entry.trim()) {
      const err = new Error('files 数组中的每一项都必须是非空字符串');
      err.statusCode = 400;
      throw err;
    }
    ...
    try {
      ...
      const err = new Error(`文件不存在: ${entry}`);
      err.statusCode = 400;
      throw err;
    }

    if (!isPathInsideDir(resolved, stageDir)) {
      const err = new Error('upload path outside stage dir');
      err.statusCode = 403;
      throw err;
    }
    ...
    if (!stat.isFile()) {
      const err = new Error(`不是普通文件: ${entry}`);
      err.statusCode = 400;
      throw err;
    }
...
if (typeof globalThis.WebSocket !== 'undefined') {
  ...
  try {
    ...
    console.error('[CDP Proxy] 错误：Node.js 版本 < 22 且无法解析 ws 模块');
    console.error('  解决方案：升级到 Node.js 22+，或让当前 Node 运行环境能够解析模块 "ws"');
    process.exit(1);
  }
}
...
async function connect() {
  ...
  if (!chromePort) {
    ...
    if (!discovered) {
      throw new Error(
        'Chrome remote debugging unavailable。请先启用可连接的浏览器调试会话：\n' +
        '  方式 1：打开 chrome://inspect/#remote-debugging 并允许当前浏览器实例\n' +
        '  方式 2：用 --remote-debugging-port 启动 Chrome / Chromium 后重试'
      );
    }
  ...
  try {
    ...
    if (pathname === '/health') {
      ...
      res.end(JSON.stringify({
        ...
        chromePort,
        fileUploadEnabled: ENABLE_FILE_UPLOAD,
        uploadStageDir: UPLOAD_STAGE_DIR,
      }));
      return;
    }
    ...
    // body: JSON { "selector": "input[type=file]", "files": ["/path/to/file1.png", "/path/to/file2.png"] }
      ...
      try {
        ...
        res.statusCode = 400;
        res.end(JSON.stringify({ error: 'POST body 需要合法 JSON' }));
        return;
      }
      ...
      try {
        ...
        res.end(JSON.stringify({
          error: e.message,
          uploadEnabled: ENABLE_FILE_UPLOAD,
          uploadStageDir: UPLOAD_STAGE_DIR,
        }));
        return;
      }
      ...
      res.end(JSON.stringify({ success: true, files: files.length, uploadStageDir: UPLOAD_STAGE_DIR }));
    }
    ...
    // GET /info?target=xxx - 获取页面信息
      ...
      res.end(JSON.stringify({
        ...
        endpoints: {
          ...
          '/setFiles?target=': 'POST body=JSON - 上传暂存区中的文件（默认关闭）',
