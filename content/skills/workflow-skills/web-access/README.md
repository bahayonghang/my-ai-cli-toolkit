...
使用仓库或 skills CLI 的受控安装路径，不要把这个 skill 写成“由 skill 自己去 clone 第三方仓库”。

例如：

```bash
npx skills add bahayonghang/my-claude-code-settings/content/skills --skill web-access
```

把 `<skill-dir>` 替换成实际安装路径时，仅用于说明本地已安装 skill 的目录结构，不作为外部仓库克隆目标。
