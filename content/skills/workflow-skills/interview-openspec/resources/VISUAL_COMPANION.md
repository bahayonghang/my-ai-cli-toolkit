...
- **Claude Code:** Default mode works — the script backgrounds the server itself
- **Codex:** Script auto-detects CODEX_CI and switches to foreground mode. Run normally.
- **Gemini CLI:** Use `--foreground` and set `is_background: true` on your shell tool call

If the URL is unreachable from the browser (common in remote/containerized setups), bind a non-loopback host:
...
- **Options (A/B/C choices):** `.options` > `.option[data-choice]` — add `data-multiselect` to container for multi-select
- **Cards (visual designs):** `.cards` > `.card[data-choice]` — with `.card-image` and `.card-body`
- **Mockup container:** `.mockup` > `.mockup-header` + `.mockup-body`
- **Split view:** `.split` — side-by-side comparison
...
When the user clicks options in the browser, interactions are recorded to `$SCREEN_DIR/.events` (one JSON object per line). The file is cleared automatically when you push a new screen.
...
Sessions using `--project-dir` persist mockup files in `.superpowers/brainstorm/` for later reference.
