...
- **禁用 Inter 字体**。`-l/-m/-i` 必须走本地 editorial 字体协议：标题优先用 Skill 自带的仓耳今楷；`-i` 的密集标签与编号再搭配 sans / mono。`-v/-c` 保持各自专用字体系统
- 任何需要截图稳定性的字体，都优先用本地资产，不依赖远程 `@import`

### 正文
- 默认：`text-base`、`leading-relaxed`、最大行宽 `65ch`
- `-l/-m` 正文优先用仓耳今楷，避免网页默认 sans 味
...
### 排版
- **禁止 Inter 字体**：editorial mode 用仓耳今楷；高密度标签用清晰 sans；只在数据短码和编号里用 mono
...
- [ ] 是否避免了三等分等宽卡片？
- [ ] 标题是否用了本地仓耳今楷或该 mode 的专用字体，而不是默认网页字体？
- [ ] 颜色是否统一冷暖调，无纯黑？
- [ ] 强调色是否 ≤ 1 个且饱和度 < 80%？
