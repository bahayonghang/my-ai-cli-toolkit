...
# ERRATUM: Acknowledged by authors in a later public revision (for arXiv, see version history)
# We implement the corrected version
scores = torch.matmul(q, k.transpose(-2, -1)) / math.sqrt(self.d_k)
```

If the source is arXiv, check version history — v2 and v3 often contain corrections. If the source is OpenReview, check whether the forum thread or updated PDF reflects revisions. If the source is a local PDF, note that revision history may be unavailable.
