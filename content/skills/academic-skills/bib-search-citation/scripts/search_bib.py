...
CODE_HINT_TERMS = ["github", "gitlab", "code", "repository", "repo", "source code", "code available"]
...
def parse_args() -> argparse.Namespace:
    ...
    input_group.add_argument("--spec-file", help="Path to a JSON file containing the search specification")
    input_group.add_argument("--query", help="Compact query expression with optional inline filters")

    parser.add_argument("--limit", type=int, help="Override result limit when using --query")
    parser.add_argument("--sort", choices=["relevance", "year_desc", "year_asc", "title"], help="Override sort mode")
    parser.add_argument("--citation-mode", choices=["none", "latex", "typst", "both"], help="Override citation mode")
    parser.add_argument("--include-raw-bib", action="store_true", help="Include raw BibTeX in results")
    parser.add_argument(
        "--return-fields",
...
def spec_from_compact_args(args: argparse.Namespace) -> Dict[str, Any]:
    ...
    if args.return_fields:
        spec["return_fields"] = [item.strip() for item in args.return_fields.split(",") if item.strip()]
    return spec
...
def strip_outer_wrappers(value: str) -> str:
    ...
    while changed and len(text) >= 2:
        changed = False
        if text.startswith("{") and text.endswith("}") and is_balanced(text[1:-1], "{", "}"):
            text = text[1:-1].strip()
            changed = True
        elif text.startswith('"') and text.endswith('"') and is_balanced_quotes(text[1:-1]):
            text = text[1:-1].strip()
            changed = True
    return text
...
def parse_query_token(token: str) -> Optional[Tuple[str, Any]]:
    ...
    if op in {":", "="}:
        return ("filter", {"field_excludes" if neg else "field_contains": {target_field: split_csv_values(value)}})
...
def entry_year(fields: Dict[str, str]) -> Optional[int]:
...
def get_flag(entry: Dict[str, Any], name: str) -> bool:
    ...
    if normalized == "eprint":
        return bool(normalize_text(fields.get("eprint", "")) or normalize_text(fields.get("archiveprefix", "")))
    if normalized == "pdf":
...
def match_filters(entry: Dict[str, Any], filters: Dict[str, Any]) -> bool:
    ...
    years_in = set(int(item) for item in (filters.get("years_in") or []))
    exclude_years = set(int(item) for item in (filters.get("exclude_years") or []))
...
def sort_results(scored: Sequence[Tuple[float, Dict[str, Any]]], sort_mode: str) -> List[Tuple[float, Dict[str, Any]]]:
    if sort_mode == "year_asc":
        return sorted(scored, key=lambda item: ((item[1].get("year") is None), item[1].get("year") or 0, item[1].get("title") or ""))
    if sort_mode == "year_desc":
        # Use -1 as sentinel so entries with no year sort last in descending order
        return sorted(scored, key=lambda item: (item[1].get("year") or -1, item[0], item[1].get("title") or ""), reverse=True)
    if sort_mode == "title":
        return sorted(scored, key=lambda item: ((item[1].get("title") or "").lower(), -(item[1].get("year") or 0)))
    return sorted(scored, key=lambda item: (item[0], item[1].get("year") or -1, item[1].get("title") or ""), reverse=True)
...
def run_search(entries: Sequence[Dict[str, Any]], spec: Dict[str, Any]) -> Dict[str, Any]:
    ...
    scored: List[Tuple[float, Dict[str, Any]]] = [(score_entry(entry, query), entry) for entry in filtered]
    if query:
