...
def parse_typst_structure(content: str) -> tuple[list[dict], dict[str, Any]]:
    ...
    for line_num, line in enumerate(lines, start=1):
        ...
        if re.match(r"^=\s+[^=]", stripped):
            ...
            current_section = {"title": section_title, "line": line_num, "slides": []}
    ...
    content_sections = [
        ...
        if not any(
            kw in s["title"].lower()
            for kw in ["introduction", "summary", "conclusion", "q&a", "reference", "引言", "总结", "结论", "问答", "参考文献"]
        )
    ]
...
def analyze_file(file_path: Path) -> dict[str, Any]:
    ...
    result = {"file": str(file_path), "engine": engine}
...
def main():
    """Main entry point."""
    parser = argparse.ArgumentParser(description="Analyze Typst Touying or LaTeX Beamer slide structure")
    parser.add_argument("file", type=Path, help="Path to slide file (.typ or .tex)")
