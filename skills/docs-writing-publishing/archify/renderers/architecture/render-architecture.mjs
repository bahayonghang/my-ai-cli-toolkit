...
const layout = {
  ...
  // 30px on top/left/right, plus 20px extra at the bottom.
  boundaryPad: 30,
...
function pathFor(conn) {
  ...
  const start = anchor(
    from,
    chosenSide(conn.fromSide, defaultFromSide(from, to)),
  );
...
function renderLegend() {
  ...
  for (const type of used) {
    ...
    parts.push(
      `        <text x="${x + 20}" y="${y}" class="t-muted" font-size="8">${TYPE_LABELS[type] || type}</text>`,
    );
