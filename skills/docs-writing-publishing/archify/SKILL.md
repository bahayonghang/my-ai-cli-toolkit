---
name: archify
description: Create technical diagrams — architecture, workflow, sequence, data-flow, lifecycle/state — as standalone HTML files with SVG graphics, dark/light theme toggle, and built-in image/SVG export. Accepts plain language or pasted Mermaid code, laid out from scratch in archify style. Also use to convert or beautify a Mermaid diagram.
...
```json
{
  ...
  "meta": {
    ...
    "subtitle": "auth and cache fallback",
  ...
  "activations": [
    { "participant": "api", "from": 190, "to": 300, "type": "backend" }
  ],
  "cards": []
}
```
...
```json
{
  ...
  "meta": {
    ...
    "subtitle": "states and terminal outcomes",
...
When Node/ajv can't run, copy `assets/template.html` and place SVG by hand. Study the worked diagram inside the template and `examples/web-app.architecture.html` for coordinate idioms, follow the Design System below, and run the self-review checklist before delivering.
