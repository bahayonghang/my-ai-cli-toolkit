...
// ---------------------------------------------------------------------------
console.log(
  "golden renders (renderer output must byte-match checked-in examples)",
);
...
for (const [mode, input, golden] of GOLDEN) {
  ...
  try {
    ...
    check(
      `${mode}: ${golden}`,
      fresh === checked,
      `fresh render differs from examples/${golden}; if the change is intentional, run "npm run render:examples" and commit the result`,
    );
  } catch (err) {
    check(
      `${mode}: ${golden}`,
...
const webAppPath = path.join(skillRoot, "examples/web-app.architecture.html");
if (!fs.existsSync(webAppPath)) {
  check(
    "web-app.architecture.html present",
    false,
    'examples/web-app.architecture.html is missing — run "npm run render:examples" and commit the result',
  );
} else {
  ...
  for (const tag of ["style", "script"]) {
    const t = blocks(template, tag).filter(
      (b) => !b.includes("[PROJECT NAME]"),
    );
    const w = blocks(webApp, tag).filter((b) => !b.includes("Sample Web App"));
    check(
      `web-app.architecture.html ${tag} blocks match template`,
      JSON.stringify(t) === JSON.stringify(w),
      "examples/web-app.architecture.html was generated from a stale template — re-render the examples",
    );
  }
}
