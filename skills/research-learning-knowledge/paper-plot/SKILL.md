...
description: >
  ...
  Two modes: from-data (pick a pre-built catalog style, fill in your numbers)
  and from-image (reproduce an uploaded paper figure as a matplotlib script).
  Use for "把数据画成论文图", "用某风格画我的数据", "复现这张图", or a named
  catalog style. Drawing only, no paper reading; route
  journal-compliance figures (IEEE/Elsevier/Nature specs) to academic-figure.
