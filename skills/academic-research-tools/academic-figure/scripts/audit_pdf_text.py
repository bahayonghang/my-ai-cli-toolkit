#!/usr/bin/env python3
"""Audit the text font sizes that a PDF figure declares with ``Tf`` operators.

This dependency-free check catches reduced mathtext superscripts and subscripts,
and any other glyph run that falls below a journal font-size floor while the
parent matplotlib ``fontsize`` still looks compliant. It reads plain and
FlateDecode content streams, which covers normal matplotlib PDF output.

Exit codes: 0 = every run is at or above the floor, 1 = at least one run is
below it, 2 = the file cannot be read or holds no auditable text run.

Provenance
----------
Ported from ``scripts/audit_pdf_text.py`` of the ``nature-figure`` skill in
``Yuan1z0825/nature-skills`` (https://github.com/Yuan1z0825/nature-skills),
reviewed 2026-08-16. Copyright the nature-skills authors.
...
Changes against the upstream file:

* renamed the report title to this skill;
* replaced the em dash in the report with an ASCII hyphen, so the output stays
  ASCII on a Windows console that is not UTF-8;
* kept the scan logic, the CLI contract, and the exit codes unchanged.
"""
...
def render_text(path: Path, result: dict[str, object]) -> str:
    lines = [
        "academic-figure PDF text audit",
        f"pdf: {path}",
        f"minimum required: {result['minimum_required_pt']:g} pt",
    ]
    if not result["auditable"]:
        lines.append(
            "verdict: NOT AUDITABLE - no supported Tf text operators were found"
        )
    else:
        lines.extend(
            [
    ...
    for run in result["below_minimum"]:
        lines.append(
            f"  - stream {run['stream']}: /{run['font']} {run['size_pt']:g} Tf"
        )
    for warning in result["warnings"]:
        lines.append(f"warning: {warning}")
    lines.append(
        "note: Tf scanning does not replace final-size visual inspection "
        "or account for every PDF transform"
    )
    return "\n".join(lines)
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Audit PDF figure text against a minimum font size in points."
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=5.0,
        help="minimum allowed Tf font size in points (default: 5)",
    )
    parser.add_argument(
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.json:
        print(
            json.dumps({"pdf": str(args.pdf), **result}, indent=2, ensure_ascii=False)
        )
    else:
