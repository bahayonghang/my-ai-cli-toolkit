...
- **Tables**: see `tables-cookbook.md` — recipe 2 (evidence table) for the source appendix, recipe 4 (comparison) when contrasting sources/positions, recipe 5 (trend) for findings that have a temporal series, recipe 8 (empty state) when a theme has no supporting evidence yet. Highlight key sources with `key-row`; use `evidence-column` for how each source supports a claim.
...
Source table, confidence badge, theme sections, uncertainty callout, recommendation cards, and citation appendix. For charts, see `charts-cookbook.md` — recipe 2 (horizontal bar) for source-confidence distribution, recipe 7 (dot plot) for timeline-of-findings across themes, recipe 8 (stacked progress) for "evidence vs inference" share of each conclusion.
...
<section id="findings" aria-labelledby="findings-title">
  ...
  <article class="card">
    ...
    <p>Quoted facts with citation [1].</p>
  </article>
  <article class="card">
    <h3 class="card-title">Inference</h3>
    <p>
      Reasoned implication, labeled as inference and not as a verified claim.
    </p>
  </article>
</section>
```
