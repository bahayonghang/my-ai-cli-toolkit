...
完成条件：筛选修复和回归测试均存在，本地筛选页面入口证明状态不再丢失，必需检查全部通过且 diff/status 证明改动在授权范围内；缺配置导致无法检查时为未完成。
...
Stop when: the filter fix and regression test exist, the local filter page entry point proves state no longer disappears, all required checks pass, and diff/status proves only authorized changes; missing configuration that prevents required checks means incomplete.
...
边界：只修改筛选状态、URL 同步、直接相关组件和回归测试，不触碰后端接口或部署配置。
迭代策略：一次做一个聚焦改动，每次改动后重跑失败检查，连续失败两次后先读日志和现有测试再换方案。
完成条件：修复和回归测试均存在，具名测试入口证明行为符合要求，全部必需检查通过且命令输出和 diff/status 范围证据出现在对话记录中；缺检查为未完成。
...
function validContract({
  ...
  pause = 'Pause or stop and report when authority conflicts, baseline drift, credentials, production data, destructive actions, or a fourth failed repair round is reached; budget exhaustion is not completion.',
} = {}) {
...
test('Trellis implementation detection is not disabled by an unrelated-edit exclusion', () => {
  const result = lintText(
    trellisDefaultGoal
      .replace('不改无关脏文件。', 'do not edit unrelated files.')
      .replaceAll(
        'python ./.trellis/scripts/task.py archive .trellis/tasks/08-22-checkout-discount',
        'archive the current task',
      ),
...
const reviewEnvelope = `scanner: node review-scan.mjs; scanner_identity: UNVERIFIED; config: config/review.json sha256 plus --strict; inputs: session IDs from reports/sessions.json; targets: src/parser/ and tests/parser/; baseline_report: reports/review-baseline.json; git_baseline: branch fix/parser plus full HEAD and dirty-scope summary. Because scanner_identity is UNVERIFIED, use bounded repeated runs and a second independent evidence source. Run the scanner and retain its command output.`;
const reviewQuestionGate = `Before the first product write, only an unresolved user-owned decision that materially changes scope, risk, cost, public behavior, or authorization may use AskUserQuestion; another host uses its actual available equivalent, and a host without a structured tool asks one concise question. Do not let same-scope findings trigger AskUserQuestion. Do not let ordinary implementation details trigger AskUserQuestion. Do not let repository-readable facts, original scanner reruns, or per-batch approvals trigger AskUserQuestion.`;
const reviewLoop = `Keep one stable finding ledger: id, severity, path_or_scope, issue, fix_required, test_required, status, evidence; statuses are open, fixed, wontfix, blocked. Finding IDs remain stable across rounds. The independent checker returns PASS | FINDINGS | BLOCKED. FINDINGS return to implementation in the same task and then an independent checker rechecks. Same-scope new findings append to the existing ledger. Allow at most three focused repair rounds.`;
const reviewCompletion = `Complete only when all conditions pass: 1. open actionable findings = 0; 2. the original scanner reruns with the same envelope; 3. regression checks pass; 4. the final gate just ci passes; 5. git diff/status proves the scope boundary.`;
const reviewPause = `If scope/config/corpus/target drift occurs, re-baseline or enter BLOCKED; a drifted comparison must never be called clean. If the same finding signature for two consecutive rounds shows no progress, enter BLOCKED. After the third round, any open actionable finding ends BLOCKED, not complete. BLOCKED reports the residual ledger and stops.`;
const noSecondPrompt = 'Do not request or emit a second repair Prompt.';

const reviewRemediationGoal = `
/goal Prefer subagents (default on); implement findings from the named review for Trellis task .trellis/tasks/08-23-parser while preserving the approved scope.
Verification: ${reviewEnvelope}
Constraints: Keep unrelated and other task directories excluded; preserve out-of-scope dirty files; keep the archive commit separate from product changes and pre-archive planning artifacts; do not push or amend; the main session does not edit product files. ${reviewQuestionGate} ${noSecondPrompt}
Boundaries: Write only under src/parser/, tests/parser/, and the current task directory; treat every other path as out of scope.
Iteration policy: Read .trellis/workflow.md Phase 2.1 / 2.2 and dispatch trellis-implement for code and trellis-check for verification. ${reviewLoop} Only after the review-remediation completion gate passes, commit this current task's related product changes and current task planning artifacts, confirm both are in version history, then run python ./.trellis/scripts/task.py archive .trellis/tasks/08-23-parser.
Stop when: ${reviewCompletion} Current-task product changes and planning artifacts are then in version history while unrelated tasks and out-of-scope dirty files remain excluded; the concrete task is archived only after those commits and any parent waits for the named release gate.
Pause if: ${reviewPause} Budget exhaustion is not completion.
`;

test('review-remediation inline profile accepts the complete one-Prompt loop', () => {
  const result = lintText(reviewRemediationGoal, ['--review-remediation', '--platform', 'codex']);
  assert.equal(result.status, 0, result.stderr);
});
...
test('review-remediation profile fails closed when any scan-envelope field is missing', () => {
  for (const [field, replacement] of [
    ['scanner', 'scan tool'],
    ['scanner_identity', 'tool provenance'],
    ['config', 'settings'],
    ['inputs', 'corpus set'],
    ['targets', 'write set'],
    ['baseline_report', 'initial artifact'],
    ['git_baseline', 'repository state'],
  ]) {
    const result = lintText(
      reviewRemediationGoal.replace(`${field}:`, `${replacement}:`),
      ['--review-remediation'],
    );
    assert.notEqual(result.status, 0, `${field} unexpectedly passed`);
    assert.match(result.stderr, new RegExp(`scan envelope missing .*${field}`, 'i'));
  }
});

test('review-remediation rejects empty or template-like scan-envelope records', () => {
  for (const [before, after, expected] of [
    ['scanner: node review-scan.mjs', 'scanner: ', /scanner.*concrete non-placeholder/i],
    ['config: config/review.json sha256 plus --strict', 'config: unknown', /config.*concrete non-placeholder/i],
    ['git_baseline: branch fix/parser plus full HEAD and dirty-scope summary', 'git_baseline: HEAD only', /git_baseline.*branch, HEAD, and dirty/i],
  ]) {
    const result = lintText(reviewRemediationGoal.replace(before, after), ['--review-remediation']);
    assert.notEqual(result.status, 0, `${before} unexpectedly passed`);
    assert.match(result.stderr, expected);
  }
});

test('review-remediation accepts a concrete scanner version instead of forcing UNVERIFIED', () => {
  const result = lintText(
    reviewRemediationGoal.replace('scanner_identity: UNVERIFIED', 'scanner_identity: version 2.4.1'),
    ['--review-remediation'],
  );
  assert.equal(result.status, 0, result.stderr);
});

test('review-remediation profile rejects broken feedback, question, and completion clusters', () => {
  const mutations = [
    [
      'FINDINGS return to implementation in the same task and then an independent checker rechecks.',
      'FINDINGS are returned as a report;',
      /must feed back to implementation/i,
    ],
    [
      'Do not request or emit a second repair Prompt.',
      'Emit a new repair Prompt for the user.',
      /second repair Prompt/i,
    ],
    [
      'Do not let same-scope findings trigger AskUserQuestion.',
      'Same-scope findings may trigger AskUserQuestion.',
      /same-scope findings must not trigger/i,
    ],
    [
      'open actionable findings = 0',
      'most findings are fixed',
      /open actionable findings = 0/i,
    ],
    [
      'the original scanner reruns with the same envelope',
      'a different scanner samples the latest corpus',
      /original scanner with the same envelope/i,
    ],
    [
      'If scope/config/corpus/target drift occurs, re-baseline or enter BLOCKED;',
      'If scope/config/corpus/target drift occurs, call the newer result clean;',
      /drift as re-baseline or `BLOCKED`/i,
    ],
    [
      'If the same finding signature for two consecutive rounds shows no progress, enter BLOCKED.',
      'Retry the same finding indefinitely.',
      /two no-progress rounds/i,
    ],
    [
      'After the third round, any open actionable finding ends BLOCKED, not complete.',
      'After the third round, remaining findings are complete.',
      /round-cap residual findings/i,
    ],
  ];
...
test('review-remediation rejects policy keywords outside their canonical Goal fields', () => {
  const displaced = reviewRemediationGoal
    .replace(`Verification: ${reviewEnvelope}`, `Verification: ${reviewEnvelope} ${reviewCompletion}`)
    .replace(`Stop when: ${reviewCompletion}`, 'Stop when: 1. The prose above exists.');
  const result = lintText(displaced, ['--review-remediation']);
  assert.notEqual(result.status, 0);
  assert.match(result.stderr, /completion.*open actionable findings = 0/i);
});

test('review-remediation validates every inline Goal block independently', () => {
  const result = lintText(`${reviewRemediationGoal}\n${baseGoalOnly}`, ['--review-remediation']);
  assert.notEqual(result.status, 0);
  assert.match(result.stderr, /\/goal\[2\].*scan envelope missing `scanner` record/i);
});

test('review-remediation rejects contradictory second-Prompt authorization', () => {
  const result = lintText(
    reviewRemediationGoal.replace(
      noSecondPrompt,
      `${noSecondPrompt} Emit a new repair Prompt for the user.`,
    ),
    ['--review-remediation'],
  );
  assert.notEqual(result.status, 0);
  assert.match(result.stderr, /must not also authorize a new repair Prompt/i);
});

test('review-remediation rejects missing same-task recheck and question exclusions', () => {
  for (const [before, after, expected] of [
    [
      'FINDINGS return to implementation in the same task and then an independent checker rechecks.',
      'FINDINGS return to implementation and stop there.',
      /same task|independent recheck/i,
    ],
    [
      'Finding IDs remain stable across rounds.',
      'Finding IDs are regenerated after every check.',
      /finding IDs must remain stable across rounds/i,
    ],
    [
      'Do not let ordinary implementation details trigger AskUserQuestion.',
      'Ordinary implementation details may trigger AskUserQuestion.',
      /ordinary implementation choices must not trigger/i,
    ],
    [
      'a host without a structured tool asks one concise question.',
      'a host without a structured tool invents a default.',
      /one concise no-tool fallback/i,
    ],
  ]) {
    const result = lintText(reviewRemediationGoal.replace(before, after), ['--review-remediation']);
    assert.notEqual(result.status, 0, before);
...
test('review-remediation keeps equivalent feedback semantics for opt-out and capability fallback', () => {
  const optOut = reviewRemediationGoal
    .replace('Prefer subagents (default on);', 'Subagents explicitly disabled by the user; use main-session inline implementation;')
    .replace('dispatch trellis-implement for code and trellis-check for verification', 'use trellis-before-dev, main-session edit, then trellis-check')
    .replace('the main session does not edit product files.', 'the main session may edit approved product files.');
  const fallback = reviewRemediationGoal
    .replace('Prefer subagents (default on);', 'Prefer subagents (default on), but because codex.dispatch_mode: inline this is a technical fallback to inline execution;')
    .replace('dispatch trellis-implement for code and trellis-check for verification', 'use trellis-before-dev, main-session edit, then trellis-check')
    .replace('the main session does not edit product files.', 'the main session may edit approved product files.');

  for (const text of [optOut, fallback]) {
    const result = lintText(text, ['--review-remediation']);
    assert.equal(result.status, 0, result.stderr);
  }
});
...
test('ordinary published examples preserve completion conjunction and remain lintable', () => {
  ...
  for (const match of examples) {
    ...
    assert.equal(result.status, 0, result.stderr);
    assert.match(match[1], /diff\/status/);
    assert.match(match[1], /必需检查全部通过|all project-required checks pass/);
    assert.match(match[1], /运行入口验证|runtime entry point/);
    assert.match(match[1], /交付均存在|every deliverable.*exists/);
  ...
  assert.match(completion, /All required checks pass/);
  assert.match(completion, /Diff\/status.*only authorized/);
  assert.doesNotMatch(completion, /checks pass or/);
  const strongExamples = playbook.split('## Strong Examples')[1].split('### Trellis Task Implementation')[0];
  const claudeVariants = [...strongExamples.matchAll(/```text\n(\/goal [^\n]+)\n```/g)];
  assert.equal(claudeVariants.length, 3);
  ...
  for (const [index, [, variant]] of claudeVariants.entries()) {
    ...
    assert.match(variant, /exit 0/);
    assert.match(variant, /diff\/status.*only authorized/);
    assert.match(variant, /otherwise stop after \d+ turns.*incomplete/);
    // These are condition-only variants of the preceding full contract.
    const fullVariant = completeExamples[index].replace(/^[^\n]+/, variant);
    const result = lintText(fullVariant, ['--platform', 'claude']);
    assert.equal(result.status, 0, result.stderr);
  }
});

test('review-remediation remains opt-in and pure read-only Trellis review does not trigger implementation gates', () => {
  const ordinary = lintText(baseGoalOnly);
  assert.equal(ordinary.status, 0, ordinary.stderr);

  const readOnlyReview = `
/goal Review only .trellis/tasks/08-23-parser and report findings without changes; do not edit or implement anything.
Verification: inspect the task artifacts and cite file and line evidence in the final report.
Constraints: remain read-only and do not execute product writes, commits, or archive actions.
Boundaries: read the named task and directly relevant source only.
Iteration policy: inspect once, cross-check evidence, and report uncertainty without repair attempts.
Stop when: 1. Every reported finding has a concrete source anchor.
Pause if: required source files are unavailable or authority conflicts.
`;
  const result = lintText(readOnlyReview);
  assert.equal(result.status, 0, result.stderr);
});
