...
3. 默认在调用 tab/cwd 创建一个 sibling，按布局选 right/down，保持 `--no-focus`。解析返回 ID，确认 shell 可用，用 `agent start` 启动指定 kind 并确认 ready。
4. 向任务自有且 idle 的 worker 发送一次完整 assignment；使用 `agent prompt --wait` 和有限 timeout。失败后检查同一 worker，不重复提交或自动替换；blocked、unknown、旧输出都不能证明完成。
