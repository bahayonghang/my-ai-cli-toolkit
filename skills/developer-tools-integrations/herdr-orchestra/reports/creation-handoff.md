...
`herdr-orchestra` **0.1.0**：在 Herdr 内委派有范围的 worker 任务，分开回传 lifecycle、响应和验收证据。源码位于 `skills/developer-tools-integrations/herdr-orchestra/`；本次未发布，也未修改已安装 skill。
...
- **validated advantage**：仅限 [output-eval](output-eval.md) 中 25 条 source-guided 候选 trace 与 assertions 的手工一致性；不是与其他工具的效果比较。
...
- `export_skill_ir.py <skill-dir> --output reports/skill-ir.json`：退出 0，已生成原始 IR；字段缺省限制见下文。
- `rtk proxy just skills-check`：退出 0，本包及当前仓库技能 metadata 检查通过。

本仓库明确用顶层 metadata、neutral interface 和 generated catalog，故不添加 per-package README/manifest 来迎合 qiaomu schema。qiaomu exporter 的部分字段只从 manifest 取值；生成 IR 的 version/owner 等 null 和默认 target_users 不代表本包声明这些值，`SKILL.md` 的 `version: 0.1.0` 才是仓库事实。保留原始生成报告，不手改成假的完整导出。
