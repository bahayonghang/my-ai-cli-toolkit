...
function appendActiveJobsTable(lines, jobs) {
  ...
  lines.push("| Job | Kind | Status | Phase | Elapsed | Codex Session ID | Summary |");
  lines.push("| --- | --- | --- | --- | --- | --- | --- |");
...
function pushJobDetails(lines, job, options = {}) {
  ...
  if ((job.status === "queued" || job.status === "running") && options.showCancelHint) {
    lines.push(`  Next: run the companion cancel command for job ${job.id} if you want to stop it.`);
  }
  if (job.status !== "queued" && job.status !== "running" && options.showResultHint) {
    lines.push(`  Next: run the companion result command for job ${job.id} to read the stored output.`);
  }
  if (job.status !== "queued" && job.status !== "running" && job.jobClass === "task" && job.write && options.showReviewHint) {
    lines.push("  Follow-up: consider a companion review or adversarial-review run before accepting the edits.");
  }
...
export function renderCancelReport(job) {
  ...
  lines.push("- Run the companion status command to confirm the updated queue.");
