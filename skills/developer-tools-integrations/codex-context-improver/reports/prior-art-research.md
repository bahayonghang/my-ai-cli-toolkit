# Codex context improver 2.0.0 prior-art research
...
The dated research above is preserved as research, not runtime proof. Its proposed mechanisms are now mapped to `references/context-audit.md`, `references/update-guidelines.md`, the compact root, and behavioral cases 12-23. The baseline old name refers to version 1.2.0 history only. Owner remains lyh/MIT; no third-party text or scripts were copied. Official/X source adoption and relay limitations are documented in `references/context-audit.md`. Current local evidence is summarized in `creation-handoff.md`; the baseline tests in task research are not 2.0.0 evidence.
