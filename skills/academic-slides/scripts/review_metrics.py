...
class SlideAnalyzer:
    ...
    def score_content_density(self) -> Dict:
        ...
        return {
            'score': score,
            'max': 10,
            'details': '; '.join(details) if details else 'Good content density'
        }
    ...
    def score_visual_balance(self) -> Dict:
        ...
        for slide in self.slides:
            ...
            has_visual = (self._has_math(slide['content']) or
                         self._has_table(slide['content']) or
                         self._has_figure(slide['content']))
        ...
        if not has_any_visual:
            ...
            self.issues.append({
                ...
                'slide': 'N/A',
                'line': 0,
                'message': 'No tables, figures, or math formulas found'
            })
        ...
        return {
            'score': score,
            'max': 10,
            'details': '; '.join(details) if details else 'Good visual balance'
        }
        ...
        try:
            ...
            if self.engine == 'typst':
                result = subprocess.run(
                    ['typst', 'compile', str(self.file_path),
                     str(output_dir / f'{self.file_path.stem}.pdf')],
                    capture_output=True,
                    timeout=30
                )
            else:
                result = subprocess.run(
                    ['latexmk', '-xelatex', '-interaction=nonstopmode',
                     f'-output-directory={output_dir}', str(self.file_path)],
                    capture_output=True,
                    timeout=60
                )
...
def main():
    parser = argparse.ArgumentParser(
        description='Compute quality scores for academic slide files'
    )
