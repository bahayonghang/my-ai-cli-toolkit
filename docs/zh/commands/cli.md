...
**描述**: 使用 Codex CLI 进行交互式代码审查，支持配置审查目标、模型和自定义指令。标准场景走 `codex review`，当固定目标与自定义关注点需要同时出现时切换到 `codex exec`。
...
3. **路由模式** - 普通目标审查走 `codex review`；若目标标志与自定义关注点需要共存，则切换到 `codex exec`
4. **构建提示** - 根据选定的关注领域（通用、安全、性能、代码质量）构建审查提示
5. **执行审查** - 通过 ccw cli 端点运行选中的 Codex CLI 命令
...
- 仅指定目标标志时，可以继续使用 `codex review`
- 目标标志与自定义关注点同时存在时，应切换到 `codex exec`
