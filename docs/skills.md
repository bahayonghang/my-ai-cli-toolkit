...
`skills/` 是一方 skill catalog。当前自动索引到 **5 个分类、24 个 skill**；每个条目都有独立详情页。
...
- [codex-dynamic-workflows](/skills/development-workflows/codex-dynamic-workflows) — Plan and run AI-agent dynamic workflows for complex tasks that benefit from explicit orchestration, goal mode, subagents or simulated work packets, approval gates, integration, verification, and reusable workflow artifacts.
