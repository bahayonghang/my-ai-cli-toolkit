...
## Skill Package Catalog And Eval Boundary

Skill packages are cross-layer artifacts. A single `SKILL.md` metadata change
flows through:

```
skills/**/SKILL.md → docs/scripts/sync_docs_catalog.py → docs/skills*.md
→ docs/.vitepress/generated/catalog.mjs → docs-check / VitePress build
```

For skill optimization work, eval metadata and review artifacts add another
boundary:

```
skills/**/evals/evals.json → local *-workspace/iteration-* grading.json
→ aggregate_benchmark.py → benchmark.json / benchmark.md → review.html
```

### Checklist: After Modifying A Public Skill

- [ ] Confirm `SKILL.md` frontmatter has `name`, `description`, `category`,
      `tags`, and `version`, with `category` matching the parent directory.
- [ ] Search the skill tree for stale local-resource or workflow references
      before final validation.
- [ ] Run `just docs-sync` after public metadata or docs-derived content
      changes, then inspect both detail pages and catalog/sidebar entries.
- [ ] Keep local skill-creator workspaces ignored unless explicitly asked to
      archive them.
- [ ] On Windows, run skill-creator aggregation/review commands with UTF-8
      mode when eval metadata contains Chinese text, e.g. `python -X utf8`.
- [ ] When Node tests spawn Python scripts that may emit Chinese output, set
      `PYTHONUTF8=1` and `PYTHONIOENCODING=utf-8` in the spawned process
      environment before asserting stderr/stdout text.
- [ ] If the aggregator writes default run counts or token labels that do not
      match the actual run shape, correct the local benchmark notes before
      generating `review.html`.

**Real-world example**: `goudi` optimization added Chinese eval prompts and a
local `goudi-workspace/iteration-1`. `aggregate_benchmark.py` failed under the
default Windows text encoding while reading `eval_metadata.json`; rerunning with
`python -X utf8` avoided changing the external tool. The generated benchmark
also needed local notes clarifying that the run was one controlled inline sample
per configuration and that the "tokens" value was an output-character proxy.
