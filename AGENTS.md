...
## 项目结构与模块组织
本仓库用于管理跨平台 AI 技能与命令。核心目录：`skills/`（本地技能模块，每个技能一个子目录含 `SKILL.md`）、`commands/`（各平台斜杠命令）、`prompts/`（全局提示词）、`external-skills/`（外部技能注册），测试位于 `tests/`，文档站点位于 `docs/`。入口脚本是 `install.py` 与 `install_tui.py`。

## 构建、测试与开发命令
常用命令如下：
```bash
python3 install_tui.py   # 启动交互式 TUI
...
## 安装与配置提示
默认安装到 Claude：`python3 install.py install-all`。跨平台目标用 `--target`（如 `--target gemini`）。项目级安装可使用 `--project ./path`。
