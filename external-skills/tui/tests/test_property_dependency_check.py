...
from core.models import DependencyCheckResult, DependencyStatus
    ...
    **Validates: Requirements 4.2, 4.3**
    ...
    def test_all_deps_satisfied(self, platform):
        ...
        **Validates: Requirements 4.3**
        ...
        # 所有依赖都存在
        env_state = {dep: True for dep in DEPENDENCIES}
        ...
        try:
            ...
            assert result.all_satisfied is True, (
                f"所有依赖满足时应返回 all_satisfied=True"
            )
        ...
        **Validates: Requirements 4.3**
        ...
        try:
            ...
            assert result.all_satisfied is False, (
                f"所有依赖不满足时应返回 all_satisfied=False"
            )
    ...
    def test_partial_deps_satisfied(self, platform):
        ...
        **Validates: Requirements 4.2, 4.3**
        ...
        try:
            ...
            assert result.all_satisfied is False, (
                f"部分依赖满足时应返回 all_satisfied=False"
            )
        ...
        **Validates: Requirements 4.2**
        ...
        try:
            ...
            assert result1.all_satisfied == result2.all_satisfied, (
                f"幂等性检查失败: all_satisfied 不一致"
            )
            
            assert len(result1.dependencies) == len(result2.dependencies), (
                f"幂等性检查失败: dependencies 长度不一致"
            )
            
            for d1, d2 in zip(result1.dependencies, result2.dependencies):
                assert d1.name == d2.name, f"幂等性检查失败: 依赖名称不一致"
